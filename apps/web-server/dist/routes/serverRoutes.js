...
router.get('/resources', authenticateToken, checkRole(STAFF_ROLES), async (req, res) => {
    let connection = null;
    try {
...
router.get('/staff', async (req, res) => {
    ...
    try {
        ...
        const getOrInitStatus = (nameInput) => {
        ...
        try {
            ...
            console.warn("[Staff Route] Query failed:", statusErr.message);
        }
        // 2. Try Plan DB
        try {
            if (!connection)
            ...
            `);
            rows.forEach((r) => getOrInitStatus(r.name).mc = true);
        }
        catch (dbErr) {
        ...
        try {
            ...
            if (settings?.value) {
                ...
                if (Array.isArray(cards)) {
                    ...
                    cards.forEach((card) => {
                        const idsToCheck = [];
                        if (Array.isArray(card.discordIds))
                            idsToCheck.push(...card.discordIds);
                        if (card.socials?.discord) {
                            card.socials.discord.split(',').forEach((p) => {
                                const trimmed = p.trim();
                                if (/^\d{17,20}$/.test(trimmed))
                                    idsToCheck.push(trimmed);
                            });
                        }
        ...
        rows.forEach((r) => {
            ...
            if (!dbUsersMap.has(low))
                dbUsersMap.set(low, { username: r.username, uuid: r.uuid, groups: [] });
            if (r.group_name)
                dbUsersMap.get(low).groups.push(r.group_name.replace('group.', ''));
        });
        const skinMap = {};
        const sessionMap = {};
        ...
        if (dbUuids.length > 0) {
            try {
                ...
                skinRows.forEach(sr => skinMap[sr.uuid] = sr.skin_identifier);
                const [sessionRows] = await connection.execute(`
                    ...
                    GROUP BY pu.uuid
                `, dbUuids);
                sessionRows.forEach(s => sessionMap[s.uuid] = s.session_start);
            }
            catch (err) {
                console.warn("Extra DB fetch failed:", err);
            }
        }
        const staff = activeUsernames.map((lowName) => {
            ...
            const panelInfo = Array.isArray(cards) ? cards.find((c) => c.name.toLowerCase() === lowName || (c.mc_nickname && c.mc_nickname.toLowerCase() === lowName)) : null;
            let role = groups.find(g => staffGroups.includes(g.toLowerCase())) || panelInfo?.role || groups[0] || 'default';
            let roleImage = null;
            if (username.toLowerCase() === 'ultraxn') {
                role = 'Neroferno';
            ...
            if (role === 'killuwu') {
                role = 'Killuwu';
                roleImage = '/ranks/rank-killu.png';
            }
            if (role === 'developer') {
                role = 'Developer';
