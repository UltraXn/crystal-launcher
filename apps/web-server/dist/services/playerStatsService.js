...
export const getPlayerStats = async (usernameFragment) => {
    ...
    const [users] = await db.query(query, [cleanUsername]);
...
const getSessionStats = async (planUserId) => {
    try {
        ...
        return { playtime: 0, mobs: 0, deaths: 0 };
    }
};
const getPvpKills = async (uuid) => {
    try {
        const [rows] = await db.query(`SELECT COUNT(*) as count FROM plan_kills WHERE killer_uuid = ?`, [uuid]);
...
export const getMoney = async (uuid) => {
    try {
        const [rows] = await db.query(`
            SELECT 
...
const getLuckPermsGroups = async (uuid) => {
    try {
        const [lpUsers] = await db.query(`SELECT primary_group FROM luckperms_players WHERE uuid = ?`, [uuid]);
        const groups = [];
        if (lpUsers.length > 0 && lpUsers[0].primary_group)
            groups.push(lpUsers[0].primary_group);
        const [lpPerms] = await db.query(`SELECT permission FROM luckperms_user_permissions WHERE uuid = ? AND permission LIKE 'group.%'`, [uuid]);
...
const getCoreProtectStats = async (username) => {
    try {
        if (!process.env.CP_DB_HOST)
            return { mined: 0, placed: 0 };
        const [users] = await cpDb.query('SELECT rowid FROM co_user WHERE user = ? LIMIT 1', [username]);
        if (users.length === 0)
            return { mined: 0, placed: 0 };
        const cpUserId = users[0].rowid;
        const [mined] = await cpDb.query('SELECT COUNT(*) as count FROM co_block WHERE user = ? AND action = 0', [cpUserId]);
        const [placed] = await cpDb.query('SELECT COUNT(*) as count FROM co_block WHERE user = ? AND action = 1', [cpUserId]);
...
const processRank = (groups) => {
    ...
    if (highestRank === 'developer')
        rankName = 'Developer';
    let rankImage = 'user.png';
    switch (highestRank) {
        case 'neroferno':
            rankImage = 'rank-neroferno.png';
            break;
        case 'fundador':
            rankImage = 'rank-fundador.png';
            break;
        case 'donador':
            rankImage = 'rank-donador.png';
            break;
        case 'developer':
            rankImage = 'developer.png';
            break;
        case 'killuwu':
            rankImage = 'rank-killu.png';
            break;
        default:
            rankImage = 'user.png';
            break;
