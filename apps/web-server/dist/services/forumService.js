...
export const createThread = async ({ category_id, title, content, user_data, poll_data }) => {
...
export const getPosts = async (threadIdOrSlug) => {
    let threadId;
...
export const createPost = async ({ thread_id, content, user_data }) => {
...
export const updateThread = async (idOrSlug, updates) => {
    let threadId;
...
export const deleteThread = async (idOrSlug) => {
    let threadId;
    if (typeof idOrSlug === 'string' && !/^\d+$/.test(idOrSlug)) {
        const { data: thread } = await supabase.from('forum_threads').select('id').eq('slug', idOrSlug).single();
        if (!thread)
