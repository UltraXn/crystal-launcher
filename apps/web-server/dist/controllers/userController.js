...
export const updateUserRole = async (req, res) => {
    try {
        ...
        const { role } = req.body;
        if (!role)
            return sendError(res, 'Role is required', 'MISSING_FIELD', 400);
...
export const updateUserMetadata = async (req, res) => {
    try {
        ...
        if (!metadata)
            return sendError(res, 'Metadata object is required', 'MISSING_FIELD', 400);
...
export const giveKarma = async (req, res) => {
    try {
        const { id } = req.params;
        const voterId = req.user.id; // From authenticateToken middleware
        if (id === voterId) {
