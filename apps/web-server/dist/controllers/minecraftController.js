...
async function syncSupabaseMetadata(userId, _token, metadata) {
    if (!supabase)
        return false;
    try {
...
export const verifyLinkCode = async (req, res) => {
    try {
        ...
        // 1. Verify Code
        const [rows] = await pool.execute('SELECT source, source_id, player_name, avatar_url, expires_at FROM universal_links WHERE code = ?', [code.toUpperCase()]);
        if (rows.length === 0) {
        ...
        if (source === 'minecraft') {
            ...
            const [existing] = await pool.execute('SELECT * FROM linked_accounts WHERE web_user_id = ?', [targetUserId]);
            if (existing.length > 0) {
                ...
                await pool.execute(`INSERT INTO linked_accounts 
                    (minecraft_uuid, minecraft_name, web_user_id, discord_id, discord_tag, gacha_balance, unlocked_tiers) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)`, [sourceId, playerName, targetUserId, oldRow.discord_id, oldRow.discord_tag, oldRow.gacha_balance, oldRow.unlocked_tiers]);
            }
            else {
                await pool.execute('INSERT INTO linked_accounts (minecraft_uuid, minecraft_name, web_user_id) VALUES (?, ?, ?)', [sourceId, playerName, targetUserId]);
            }
        }
...
export const initWebLink = async (req, res) => {
    try {
        ...
        await pool.execute('INSERT INTO universal_links (code, source, source_id, expires_at) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE expires_at = ?', [code, 'web', userId, expiresAt, expiresAt]);
                ...
                if (discordIdentity) {
                    ...
                    if (rows.length > 0) {
                        ...
                        if (link.discord_id !== discordId || link.discord_tag !== discordTag) {
                            ...
                            const [newRows] = await pool.execute('SELECT * FROM linked_accounts WHERE web_user_id = ?', [userId]);
                            if (newRows.length > 0)
                                rows[0] = newRows[0];
                        }
                    }
                    ...
                    if (discordAvatar) {
                        await syncSupabaseMetadata(userId, req.headers.authorization?.split(' ')[1], {
...
export const unlinkAccount = async (req, res) => {
    try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const minecraftName = rows[0]?.minecraft_name;
        ...
        try {
            ...
            if (token && sbUrl && sbAnonKey) {
                const userSupabase = createClient(sbUrl, sbAnonKey, { global: { headers: { Authorization: `Bearer ${token}` } } });
