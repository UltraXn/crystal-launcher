...
const sendNewsWebhook = async (news) => {
    if (!NEWS_WEBHOOK_URL) {
...
export const deleteComment = async (req, res) => {
    try {
        ...
        const { data: comment, error: fetchError } = await supabase
            ...
            .single();
        if (fetchError)
            console.error(`[deleteComment] DB Error fetching comment:`, fetchError);
        if (!comment)
            console.error(`[deleteComment] Comment not found in DB`);
        if (fetchError || !comment)
            return res.status(404).json({ error: "Comentario no encontrado" });
...
export const updateNews = async (req, res) => {
    try {
        ...
        const updates = { title, category, content, image, status, title_en, content_en };
        if (slug)
            updates.slug = slug;
        const { data, error } = await supabase
            .from('news')
