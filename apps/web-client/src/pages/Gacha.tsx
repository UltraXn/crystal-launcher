        ...
        try {
            ...
            if (response.success && response.data) {
                ...
                if (response.error?.code === 'COOLDOWN') {
                     setCooldown(true);
                     setError(t('gacha.already_claimed', "¡Ya jugaste hoy! Vuelve mañana."));
                } else {
                     setError(response.error?.message || t('gacha.roll_error', "Error al tirar"));
                }
            }
                ...
                <AnimatePresence mode="wait">
                    {reward ? (
                        <Motion.div 
                            ...
                            style={{ borderColor: reward.color, boxShadow: `0 0 30px ${reward.color}40` } as React.CSSProperties}
                        >
                            ...
                            {!isOpening && (
                                <>
                                    <button 
                                        ...
                                        onClick={handleOpen} 
                                        disabled={cooldown}
                                        title={cooldown ? t('gacha.vuelve_mañana', 'Vuelve mañana') : t('gacha.claim_btn', "Reclamar Diario")}
                                    >
