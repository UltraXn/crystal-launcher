        ...
        <div className="wiki-manager">
            ...
            <div className="manager-header">
                ...
                <button className="btn-primary" onClick={startNew}>
                    <FaPlus /> Crear Artículo
                </button>
            </div>

            {loading ? (
                <Loader text="Cargando biblioteca..." />
                    ...
                    {filteredArticles.map(article => (
                        <div key={article.id} className="article-card">
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', fontSize: '0.75rem', color: '#444' }}>
                                <span className="flex items-center gap-1"><FaGlobe /> Público</span>
                                <span className="flex items-center gap-1"><FaTag /> {article.content.length} caracteres</span>
                            </div>
                        </div>
                    ))}
                </div>
                    ...
                    >
                        ...
                        >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                                <h2>{editingId ? 'Editar Artículo' : 'Nuevo Artículo Wiki'}</h2>
                            ...
                            <form onSubmit={handleSave}>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                                    <div className="input-group">
                                        ...
                                        <input 
                                            ...
                                            placeholder="Ej: Guía de Encantamientos"
                                        />
                                    </div>
                                    <div className="input-group">
                                        <label>URL Slug (Único)</label>
                                        <input 
                                            type="text" 
                                            required
                                            value={formData.slug} 
                                ...
                                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
                                    <button type="button" onClick={() => setIsEditing(false)} style={{ padding: '0.8rem 2rem', color: '#666' }}>Cancelar</button>
                                    <button type="submit" disabled={saving} className="btn-primary" style={{ padding: '0.8rem 3rem' }}>
                                        {saving ? <Loader size={16} /> : <><FaSave /> {editingId ? 'Actualizar' : 'Publicar'}</>}
                                    </button>
                                </div>
                            </form>
                        </motion.div>
