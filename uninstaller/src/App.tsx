...
export function App() {
  ...
  const handleRunUninstall = async () => {
    ...
    try {
      ...
      await invoke("perform_uninstallation", { targetDir: installPath });

      await new Promise((r) => setTimeout(r, 800));

      try {
        await invoke("schedule_self_deletion", { targetDir: installPath });
      } catch {}
      ...
      >
        <div
          ...
          style={{
            ...
            borderRadius: 20,
            boxShadow: "0 20px 48px rgba(0,0,0,0.65), 0 0 32px rgba(239, 68, 68, 0.15)",
            display: "flex",
            flexDirection: "column",
            alignItems: "center",
            textAlign: "center",
        ...
        >
          ...
          <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, letterSpacing: "-0.01em" }}>
            {step === "confirm" && "¿Desinstalar CTLauncher?"}
          ...
          <p style={{ margin: "6px 0 20px 0", fontSize: 13, color: "rgba(255,255,255,0.55)", lineHeight: 1.45 }}>
            {step === "confirm" && "Se eliminarán los archivos del cliente, accesos directos y registros de tu equipo."}
            {step === "uninstalling" && "Limpiando accesos directos, registro de Windows y archivos locales..."}
            {step === "done" && "CTLauncher se ha eliminado correctamente de este equipo."}
              ...
              >
                <div
                  style={{
                    ...
                    animation: "octopusFloat 1.5s ease-in-out infinite",
                  }}
                />
              </div>
            </div>
